...
class _ProfileInformationScreenState extends State<ProfileInformationScreen> {
  ...
  // No consent modal here. registerProfile() below sends only the details the
  // user gave at sign-up, none of which the data-sharing consent names, and it
  // is the call that puts their name on their profile — a cooperative cannot
  // accept a membership application from a member who has none, so this step
  // must go through whatever they later decide about sharing. The consent is
  // collected on bank_information_screen (step 2), where the BVN is typed, and
  // kycsvc gates tier1/tier2 on it there.
  @override
  void initState() {
    super.initState();
  ...
  Widget _buildDropdown({
    ...
    Future<void> openPicker() async {
      ...
      final selected = await showModalBottomSheet<String>(
        ...
        // Sized by its contents, not by a fraction of the screen: one country
        // is one row, and the sheet grows as regions are added until it hits
        // the cap and starts scrolling.
